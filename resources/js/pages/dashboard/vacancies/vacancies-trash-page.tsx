...
import { fetchTrashedVacanciesAction } from '@/store/vacancies-slice/vacancies-api-actions';
import { getTrashedVacancies } from '@/store/vacancies-slice/vacancies-selector';
...
function VacanciesTrashPage(): ReactNode {
  const dispatch = useAppDispatch();
  const vacancies = useAppSelector(getTrashedVacancies);
  const companies = useAppSelector(getCompanies);
  const navigate = useNavigate();

  useEffect(() => {
    if (!vacancies) dispatch(fetchTrashedVacanciesAction());
...
export default VacanciesTrashPage;
