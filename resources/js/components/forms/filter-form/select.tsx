  ...
  return (
    ...
    >
      ...
      >
        ...
        <Icons.chevronDown
          className={classNames(
            'flex min-w-3 transition-all duration-300 transform',
            isOpen && 'scale-[-1]'
          )}
          width={14}
        />
      ...
      <div
        ref={menuRef}
        className={classNames(
          'absolute left-0 top-full z-20 flex flex-col gap-1 p-1 w-full max-h-56 overflow-y-scroll scrollbar-y border-r border-b border-l transition-all transform duration-150 rounded-b bg-white shadow',
      ...
      >
        {props.options.map((option) => (
          <button
            key={option.value}
            className={classNames(
              'flex items-center py-2 px-4 transition-all duration-300 hover:bg-gray-100 rounded text-left',
